  ...
  if (completedAppointments) {
    for (const appt of completedAppointments) {
      ...
      notifications.push({
        ...
        description: `${reasonLabel} for ${pet?.name || 'Unknown'}${pet?.species ? ` (${pet.species})` : ''} completed on ${new Date(appt.scheduled_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' })}. Record updated.`,
        time: formatRelativeTime(appt.scheduled_at),
        timeISO: appt.scheduled_at,
        read: false,
        petName: pet?.name,
  ...
  if (cancelledAppointments) {
    for (const appt of cancelledAppointments) {
      ...
      notifications.push({
        ...
        description: `${pet?.name || 'Unknown'}'s ${appt.reason || 'appointment'} with ${ownerLabel} on ${new Date(appt.scheduled_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' })} has been cancelled.`,
        time: formatRelativeTime(appt.scheduled_at),
        timeISO: appt.scheduled_at,
        read: false,
        petName: pet?.name,
  ...
  if (noShowAppointments) {
    for (const appt of noShowAppointments) {
      ...
      notifications.push({
        ...
        description: `${pet?.name || 'Unknown'} with ${ownerLabel} did not attend their ${appt.reason || 'appointment'} on ${new Date(appt.scheduled_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' })}. Consider reaching out to reschedule.`,
        time: formatRelativeTime(appt.scheduled_at),
        timeISO: appt.scheduled_at,
        read: false,
        petName: pet?.name,
