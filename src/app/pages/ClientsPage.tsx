  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const { data } = await db.from('staff').select('id, profiles:profiles!staff_profile_id_fkey(first_name, last_name)').eq('organization_id', organizationId).eq('role', 'veterinarian');
    ...
    <div className="max-w-[1440px] mx-auto p-8">
      ...
      {selectMode && (
        <div className="flex items-center gap-3 mb-4 px-4 py-3 border border-[var(--border-color)] bg-[var(--surface-elevated)]" style={{ borderRadius: '10px' }}>
          ...
          <button
            onClick={() => { setSelectedIds(new Set()); setSelectedRowsMeta({}); setAllAcrossPagesSelected(false); setSelectMode(false); }}
